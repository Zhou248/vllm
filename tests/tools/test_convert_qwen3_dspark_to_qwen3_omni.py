...
def test_converts_hidden_features_and_vocab_for_omni(tmp_path: Path) -> None:
    ...
    assert config["vocab_size"] == 9
    assert config["draft_vocab_size"] == 6
    assert config["mask_token_id"] == 8
    assert config["dflash_config"]["mask_token_id"] == 8
    assert config["dflash_config"]["use_aux_hidden_state"] is True

    weights = _load_weights(output_dir)
    expected_fc = source["fc.weight"].reshape(4, 2, 4)[:, :, :3].reshape(4, 6)
    torch.testing.assert_close(weights["fc.weight"], expected_fc)
    torch.testing.assert_close(
        weights["embed_tokens.weight"][:6], source["embed_tokens.weight"]
    )
    assert torch.count_nonzero(weights["embed_tokens.weight"][6:]) == 0
    assert weights["markov_head.markov_w1.weight"].shape == (9, 2)
    assert weights["lm_head.weight"].shape == (6, 4)
    assert weights["d2t"].shape == (6,)
    assert weights["d2t"].dtype == torch.int64
...
def test_generates_incompatible_shared_weights_for_smoke_test(tmp_path: Path) -> None:
    ...
    weights = _load_weights(output_dir)
    assert weights["embed_tokens.weight"].shape == (9, 4)
    assert weights["lm_head.weight"].shape == (6, 4)
...
def test_writes_loadable_sharded_checkpoint_index(tmp_path: Path) -> None:
    ...
    weights = _load_weights(output_dir)
    assert weights["fc.weight"].shape == (4, 6)
    assert weights["d2t"].shape == (6,)

...
def test_qwen3_omni_exposes_eagle3_to_its_text_backbone():
    """DSpark configures aux layers through the outer multimodal model."""
...
def test_qwen3_omni_text_model_collects_post_deepstack_aux_hidden_states():
    """The Omni override must preserve Qwen3-MoE's Eagle3 output contract."""
    from vllm.model_executor.models.qwen3_omni_moe_thinker import (
        Qwen3MoeLLMModel,
    )
    ...
    # Layer output 2 + deepstack 3 + residual 10.
...
def test_qwen3_omni_reduced_vocab_dspark_requires_mapping_and_head():
    ...
    object.__setattr__(
        model,
        "config",
        SimpleNamespace(vocab_size=100, draft_vocab_size=40),
    )
    object.__setattr__(model, "target_vocab_size", 100)

    with pytest.raises(ValueError, match="must include lm_head weights"):
        model.load_weights([])

    with pytest.raises(ValueError, match="must include a d2t mapping"):
...
def test_qwen3_omni_expanded_dspark_input_vocab_requires_embedding():
    ...
    object.__setattr__(
        model,
        "config",
        SimpleNamespace(vocab_size=101, draft_vocab_size=100),
    )
    object.__setattr__(model, "target_vocab_size", 100)

    with pytest.raises(ValueError, match="must include embed_tokens weights"):
        model.load_weights([])
...
def test_dspark_probabilistic_buffer_uses_target_output_vocab():
    ...
    def init_parent(self, vllm_config, device):
        ...
        self.draft_model_config = SimpleNamespace(
            hf_config=SimpleNamespace(dspark_bonus_anchor=False),
